...
{{- define "common.errors.upgrade.passwords.empty" -}}
  ...
  {{- $errorString = print $errorString "\n                 Further information can be obtained at https://helmhub.io/docs/troubleshooting#credential-errors-while-upgrading-chart-releases" -}}
    {{- $errorString = print $errorString "\n%s" -}}
...
{{- range .images -}}
  ...
  {{- if not (contains $fullImageNameNoTag $originalImages) -}}
    ...
    {{- else if not (contains .repository $originalImages) -}}
      ...
      {{- if contains "docker.io/helmhubio/" $fullImageNameNoTag -}}
...
{{- else if (or (gt (len $relocatedImages) 0) (gt (len $replacedImages) 0)) -}}
  ...
  {{- if and (eq (len $relocatedImages) 0) (eq (len $replacedImages) (len $bitnamiLegacyImages)) -}}
    {{- $errorString = print "\n\n⚠ WARNING: " $errorString -}}
    {{- print $errorString -}}
  {{- else if or (contains "docker.io/helmhubio/" $originalImages) (contains "docker.io/helmhubio/" $originalImages) (contains "docker.io/helmhubio/" $originalImages) -}}
    {{- $errorString = print "\n\n⚠ ERROR: " $errorString -}}
    ...
    {{- $errorString = print $errorString "\nFurther information can be obtained at https://github.com/helmhub-io/charts/issues/30850" -}}
