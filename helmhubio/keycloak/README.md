...
# HelmHubIO package for Keycloak
...
```console
helm install my-release oci://registry-1.docker.io/helmhubiocharts/keycloak
```

Looking to use Keycloak in production? Try [VMware Tanzu Application Catalog](https://bitnami.com/enterprise), the commercial edition of the HelmHubIO catalog.
...
This will import all `*.json` under `/opt/helmhubio/keycloak/data/import` files as a realm into keycloak as per the official documentation [here](https://www.keycloak.org/server/importExport#_importing_a_realm_from_a_directory). You can supply the files by mounting a volume e.g. with docker compose as follows:

```yaml
keycloak:
  image: helmhubio/keycloak:latest
  volumes:
    - /local/path/to/realms/folder:/opt/helmhubio/keycloak/data/import
```
...
The [HelmHubIO Keycloak](https://github.com/helmhub-io/containers/tree/main/helmhubio/keycloak) image allows you to use your custom scripts to initialize a fresh instance. In order to execute the scripts, you can specify custom scripts using the `initdbScripts` parameter as dict.
...
> Note: You need to substitute the placeholders `REGISTRY_NAME` and `REPOSITORY_NAME` with a reference to your Helm chart registry and repository. For example, in the case of HelmHubIO, you need to use `REGISTRY_NAME=registry-1.docker.io` and `REPOSITORY_NAME=helmhubiocharts`.
> **Tip**: You can use the default [values.yaml](https://github.com/helmhub-io/charts/tree/main/helmhubio/keycloak/values.yaml)
...
This version includes some important refactoring to remove technical debt. As a consequence, some deprecated parameters were removed and it's likely some others were renamed to match the standards used in the HelmHubIO Charts catalog.
...
helm upgrade keycloak helmhubio/keycloak \
