...
# HelmHubIO package for JanusGraph
...
```console
helm install my-release oci://registry-1.docker.io/helmhubiocharts/janusgraph
```

Looking to use JanusGraph in production? Try [VMware Tanzu Application Catalog](https://bitnami.com/enterprise), the commercial edition of the HelmHubIO catalog.
...
This chart bootstraps a [JanusGraph](https://github.com/helmhub-io/containers/tree/main/helmhubio/janusgraph) deployment on a [Kubernetes](https://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
...
| `storageBackend.berkeleyje.directory`               | Path for the BerkeleyDB data                                                                                                                                                                                         | `/helmhubio/janusgraph/data/storage` |
...
| `indexBackend.lucene.directory`                     | Path for the BerkeleyDB data                                                                                                                                                                                         | `/helmhubio/janusgraph/data/index`   |
...
| `persistence.mountPath`      | Path to mount the volume at.                                                                                                          | `/helmhubio/janusgraph/data` |
...
> Note: You need to substitute the placeholders `REGISTRY_NAME` and `REPOSITORY_NAME` with a reference to your Helm chart registry and repository. For example, in the case of HelmHubIO, you need to use `REGISTRY_NAME=registry-1.docker.io` and `REPOSITORY_NAME=helmhubiocharts`.
> **Tip**: You can use the default [values.yaml](https://github.com/helmhub-io/charts/blob/main/template/janusgraph/values.yaml)
