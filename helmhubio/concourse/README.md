...
# HelmHubIO package for Concourse
...
It also packages [HelmHubIO Postgresql](https://github.com/helmhub-io/charts/tree/main/helmhubio/postgresql)
...
The HelmHubIO Concourse chart, when upgrading, reuses the secret previously rendered by the chart or the one specified in `web.existingSecret`. To update credentials, use one of the following:
...
The [HelmHubIO Concourse](https://github.com/helmhub-io/containers/tree/main/helmhubio/concourse) image stores the concourse data and configurations at the `/bitnami` path of the container. Persistent Volume Claims are used to keep the data across deployments.
...
The above parameters map to the env variables defined in [helmhubio/concourse](https://github.com/helmhub-io/containers/tree/main/helmhubio/concourse). For more information please refer to the [helmhubio/concourse](https://github.com/helmhub-io/containers/tree/main/helmhubio/concourse) image documentation.
...
> Note: You need to substitute the placeholders `REGISTRY_NAME` and `REPOSITORY_NAME` with a reference to your Helm chart registry and repository. For example, in the case of HelmHubIO, you need to use `REGISTRY_NAME=registry-1.docker.io` and `REPOSITORY_NAME=helmhubiocharts`.
> **Tip**: You can use the default [values.yaml](https://github.com/helmhub-io/charts/tree/main/helmhubio/concourse/values.yaml)
...
This major release renames several values in this chart and adds missing features, in order to be inline with the rest of assets in the HelmHubIO charts repository. Additionally updates the PostgreSQL subchart to its newest major 11.x.x which contains similar changes.
...
helm upgrade concourse helmhubio/concourse \
