...
# HelmHubIO package for Redis&reg; Cluster
...
```console
helm install my-release oci://registry-1.docker.io/helmhubiocharts/redis-cluster
```

Looking to use Redisreg; Cluster in production? Try [VMware Tanzu Application Catalog](https://bitnami.com/enterprise), the commercial edition of the HelmHubIO catalog.
...
This chart bootstraps a [Redis&reg;](https://github.com/helmhub-io/containers/tree/main/helmhubio/redis) deployment on a [Kubernetes](https://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
...
While [Redis&reg; Helm Chart](https://github.com/helmhub-io/charts/tree/main/helmhubio/redis) will deploy a master-slave cluster using Redis&reg; Sentinel, the [Redis&reg; Cluster Helm Chart](https://github.com/helmhub-io/charts/tree/main/helmhubio/redis-cluster) will deploy a Redis&reg; Cluster with sharding.
...
```console
$ cat /helmhubio/redis/data/nodes.sh
...
See [#15075](https://github.com/helmhub-io/charts/issues/15075)
...
| `persistence.path`                                          | Path to mount the volume at, to use other images Redis&reg; images.                                                                                                                                                                                   | `/helmhubio/redis/data`           |
...
> Note: You need to substitute the placeholders `REGISTRY_NAME` and `REPOSITORY_NAME` with a reference to your Helm chart registry and repository. For example, in the case of HelmHubIO, you need to use `REGISTRY_NAME=registry-1.docker.io` and `REPOSITORY_NAME=helmhubiocharts`.
> **Tip**: You can use the default [values.yaml](https://github.com/helmhub-io/charts/tree/main/helmhubio/redis-cluster/values.yaml)
...
This version of the chart adapts the chart to the most recent HelmHubIO best practices and standards. Most of the Redis&reg; parameters were moved to the `redis` values section (such as extraEnvVars, sidecars, and so on). No major issues are expected during the upgrade.
