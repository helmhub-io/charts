...
# HelmHubIO package for ScyllaDB
...
```console
helm install my-release oci://registry-1.docker.io/helmhubiocharts/scylladb
```

Looking to use ScyllaDB in production? Try [VMware Tanzu Application Catalog](https://bitnami.com/enterprise), the commercial edition of the HelmHubIO catalog.
...
| `persistence.mountPath`              | The path the data volume will be mounted at                                                                                                         | `/helmhubio/scylladb` |
...
| `persistence.commitLog.mountPath`    | The path the commit log volume will be mounted at. Unset by default. Set it to '/helmhubio/scylladb/commitlog' to enable a separate commit log volume | `""`                |
...
> Note: You need to substitute the placeholders `REGISTRY_NAME` and `REPOSITORY_NAME` with a reference to your Helm chart registry and repository. For example, in the case of HelmHubIO, you need to use `REGISTRY_NAME=registry-1.docker.io` and `REPOSITORY_NAME=helmhubiocharts`.
> **Tip**: You can use the default [values.yaml](https://github.com/helmhub-io/charts/tree/main/helmhubio/scylladb/values.yaml)
