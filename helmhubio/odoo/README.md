...
```console
helm install my-release oci://registry-1.docker.io/helmhubiocharts/odoo
```

Looking to use Odoo in production? Try [VMware Tanzu Application Catalog](https://bitnami.com/enterprise), the commercial edition of the HelmHubIO catalog.
...
This chart bootstraps a [Odoo](https://github.com/helmhub-io/containers/tree/main/helmhubio/odoo) deployment on a [Kubernetes](https://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
...
The [HelmHubIO Odoo](https://github.com/helmhub-io/containers/tree/main/helmhubio/odoo) image stores the Odoo data and configurations at the `/helmhubio/odoo` path of the container.
...
The above parameters map to the env variables defined in [helmhubio/odoo](https://github.com/helmhub-io/containers/tree/main/helmhubio/odoo). For more information please refer to the [helmhubio/odoo](https://github.com/helmhub-io/containers/tree/main/helmhubio/odoo) image documentation.
...
> Note: You need to substitute the placeholders `REGISTRY_NAME` and `REPOSITORY_NAME` with a reference to your Helm chart registry and repository. For example, in the case of HelmHubIO, you need to use `REGISTRY_NAME=registry-1.docker.io` and `REPOSITORY_NAME=helmhubiocharts`.
> **Tip**: You can use the default [values.yaml](https://github.com/helmhub-io/charts/tree/main/helmhubio/odoo/values.yaml)
...
Also, this major release bumps the PostgreSQL chart version to [15.x.x](https://github.com/helmhub-io/charts/pull/24171).
...
helm upgrade odoo helmhubio/odoo \
...
The [HelmHubIO Odoo](https://github.com/helmhub-io/containers/tree/main/helmhubio/odoo) image was refactored and now the source code is published in GitHub in the `rootfs` folder of the container image repository.
...
- The different fields present in the *Chart.yaml* file has been ordered alphabetically in a homogeneous way for all the HelmHubIO Helm Chart.
