...
```console
helm install my-release oci://registry-1.docker.io/helmhubiocharts/jenkins
```

Looking to use Jenkins in production? Try [VMware Tanzu Application Catalog](https://bitnami.com/enterprise), the commercial edition of the HelmHubIO catalog.
...
This chart bootstraps a [Jenkins](https://github.com/helmhub-io/containers/tree/main/helmhubio/jenkins) deployment on a [Kubernetes](https://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
...
The [HelmHubIO Jenkins](https://github.com/helmhub-io/containers/tree/main/helmhubio/jenkins) image stores the Jenkins data and configurations at the `/helmhubio/jenkins` path of the container. Persistent Volume Claims (PVCs) are used to keep the data across deployments.
...
| `jenkinsHome`            | Jenkins home directory                                                                                                                              | `/helmhubio/jenkins/home` |
...
| `skipImagePlugins`       | Set this value to true to skip installing plugins stored under /opt/helmhubio/jenkins/plugins                                                         | `false`                 |
...
The above parameters map to the env variables defined in [helmhubio/jenkins](https://github.com/helmhub-io/containers/tree/main/helmhubio/jenkins). For more information please refer to the [helmhubio/jenkins](https://github.com/helmhub-io/containers/tree/main/helmhubio/jenkins) image documentation.
...
> Note: You need to substitute the placeholders `REGISTRY_NAME` and `REPOSITORY_NAME` with a reference to your Helm chart registry and repository. For example, in the case of HelmHubIO, you need to use `REGISTRY_NAME=registry-1.docker.io` and `REPOSITORY_NAME=helmhubiocharts`.
> **Tip**: You can use the default [values.yaml](https://github.com/helmhub-io/charts/tree/main/helmhubio/jenkins/values.yaml)
...
This major release is no longer contains the metrics section because the container `helmhubio/enkins-exporter` has been deprecated due to the upstream project is not maintained.
...
Due to recent changes in the container image (see [Notable changes](https://github.com/helmhub-io/containers/tree/main/helmhubio/jenkins#notable-changes)), the major version of the chart has been bumped preemptively.
...
```console
helm upgrade jenkins oci://REGISTRY_NAME/REPOSITORY_NAME/jenkins --set jenkinsPassword=$JENKINS_PASSWORD --set jenkinsHome=/helmhubio/jenkins/jenkins_home
kubectl exec -it $(kubectl get pod -l app.kubernetes.io/instance=jenkins,app.kubernetes.io/name=jenkins -o jsonpath="{.items[0].metadata.name}") -- find /helmhubio/jenkins -mindepth 1 -maxdepth 1 -not -name jenkins_home -exec rm -rf {} \;
```
