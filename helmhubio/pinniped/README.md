...
# HelmHubIO package for Pinniped
...
```console
helm install my-release oci://registry-1.docker.io/helmhubiocharts/pinniped
```

Looking to use Pinniped in production? Try [VMware Tanzu Application Catalog](https://bitnami.com/enterprise), the commercial edition of the HelmHubIO catalog.
...
The [HelmHubIO pinniped](https://github.com/helmhub-io/containers/tree/main/helmhubio/pinniped) image stores the pinniped data and configurations at the `/bitnami` path of the container. Persistent Volume Claims are used to keep the data across deployments. This is known to work in GCE, AWS, and minikube.
...
The above parameters map to the env variables defined in [helmhubio/pinniped](https://github.com/helmhub-io/containers/tree/main/helmhubio/pinniped). For more information please refer to the [helmhubio/pinniped](https://github.com/helmhub-io/containers/tree/main/helmhubio/pinniped) image documentation.
...
> Note: You need to substitute the placeholders `REGISTRY_NAME` and `REPOSITORY_NAME` with a reference to your Helm chart registry and repository. For example, in the case of HelmHubIO, you need to use `REGISTRY_NAME=registry-1.docker.io` and `REPOSITORY_NAME=helmhubiocharts`.
> **Tip**: You can use the default [values.yaml](https://github.com/helmhub-io/charts/tree/main/helmhubio/pinniped/values.yaml)
