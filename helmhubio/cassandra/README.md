...
# HelmHubIO package for Apache Cassandra
...
```console
helm install my-release oci://registry-1.docker.io/helmhubiocharts/cassandra
```

Looking to use Apache Cassandra in production? Try [VMware Tanzu Application Catalog](https://bitnami.com/enterprise), the commercial edition of the HelmHubIO catalog.
...
Refer to our detailed tutorial on [backing up and restoring HelmHubIO Apache Cassandra deployments on Kubernetes](https://techdocs.broadcom.com/us/en/vmware-tanzu/application-catalog/tanzu-application-catalog/services/tac-doc/apps-tutorials-backup-restore-data-cassandra-kubernetes-index.html).
...
| `persistence.mountPath`          | The path the data volume will be mounted at                                                                                                          | `/helmhubio/cassandra` |
| `persistence.commitLogMountPath` | The path the commit log volume will be mounted at. Unset by default. Set it to '/helmhubio/cassandra/commitlog' to enable a separate commit log volume | `""`                 |
...
> Note: You need to substitute the placeholders `REGISTRY_NAME` and `REPOSITORY_NAME` with a reference to your Helm chart registry and repository. For example, in the case of HelmHubIO, you need to use `REGISTRY_NAME=registry-1.docker.io` and `REPOSITORY_NAME=helmhubiocharts`.
> **Tip**: You can use the default [values.yaml](https://github.com/helmhub-io/charts/tree/main/helmhubio/cassandra/values.yaml)
...
This release changes uses HelmHubIO Cassandra container `3.11.4-debian-9-r188`, based on Bash.
