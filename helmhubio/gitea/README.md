...
```console
helm install my-release oci://registry-1.docker.io/helmhubiocharts/gitea
```

Looking to use Gitea in production? Try [VMware Tanzu Application Catalog](https://bitnami.com/enterprise), the commercial edition of the HelmHubIO catalog.
...
This chart bootstraps a [Gitea](https://github.com/helmhub-io/containers/tree/main/helmhubio/gitea) deployment on a [Kubernetes](https://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.

It also packages the [HelmHubIO PostgreSQL chart](https://github.com/helmhub-io/charts/tree/main/helmhubio/postgresql) which is required for bootstrapping a PostgreSQL deployment as a database for the Gitea application.
...
The [HelmHubIO Gitea](https://github.com/helmhub-io/containers/tree/main/helmhubio/gitea) image stores the Gitea data and configurations at the `/helmhubio/gitea` path of the container.
...
> Note: You need to substitute the placeholders `REGISTRY_NAME` and `REPOSITORY_NAME` with a reference to your Helm chart registry and repository. For example, in the case of HelmHubIO, you need to use `REGISTRY_NAME=registry-1.docker.io` and `REPOSITORY_NAME=helmhubiocharts`.
> **Tip**: You can use the default [values.yaml](https://github.com/helmhub-io/charts/tree/main/helmhubio/gitea/values.yaml)
