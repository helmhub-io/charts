...
# HelmHubIO package for PostgreSQL HA
...
```console
helm install my-release oci://registry-1.docker.io/helmhubiocharts/postgresql-ha
```

Looking to use PostgreSQL HA in production? Try [VMware Tanzu Application Catalog](https://bitnami.com/enterprise), the commercial edition of the HelmHubIO catalog.
...
This [Helm](https://github.com/kubernetes/helm) chart installs [PostgreSQL](https://www.postgresql.org/) with HA architecture in a Kubernetes cluster. Welcome to [contribute](https://github.com/helmhub-io/charts/blob/main/CONTRIBUTING.md) to Helm Chart for PostgreSQL HA.

This Helm chart has been developed based on [helmhubio/postgresql](https://github.com/helmhub-io/charts/tree/main/helmhubio/postgresql) chart but including some changes to guarantee high availability such as:
...
- Replacing `helmhubio/postgresql` with `helmhubio/postgresql-repmgr` which includes and configures [repmgr](https://repmgr.org/). Repmgr ensures standby nodes assume the primary role when the primary node is unhealthy.
...
The following diagram shows you the options you have for using HelmHubIO's PostgreSQL solutions in your deployments:
...
The HelmHubIO postgresql-ha chart allows configuring the securitization of both types of traffic using TLS.
...
| `metrics.serviceMonitor.selector`                | Defaults to what's used if you follow CoreOS Prometheus Install Instructions (<https://github.com/helmhub-io/charts/tree/main/helmhubio/kube-prometheus#tldr>)                                                                         | `{}`                                |
...
| `persistence.mountPath`                            | The path the volume will be mounted at, useful when using different PostgreSQL images.  | `/helmhubio/postgresql` |
...
This major version includes a security fix for [GHSA-mx38-x658-5fwj](https://github.com/helmhub-io/charts/security/advisories/GHSA-mx38-x658-5fwj) and CVE-2025-22248. Upgrading to this or higher versions is recommended.
...
kubectl run pg-client --rm --tty -i --restart='Never' --image docker.io/helmhubio/postgresql-repmgr:17 --env="PGPASSWORD=$POSTGRES_PASSWORD" --command -- \
...
```console
$ helm upgrade mypg oci://registry-1.docker.io/helmhubiocharts/postgresql-ha \
...
This major version adapts the NetworkPolicy objects to the most recent HelmHubIO standards. Now there is a separate object for `pgpool` and for `postgresql`, being located in their corresponding sections. It is also enabled by default in other to comply with the best security standards.
...
> Note: Due to an error in our release process, the latest version in the previous major branch (11.9.8) already uses 16 by default, see [PR#19590](https://github.com/helmhub-io/charts/pull/19590)
...
HelmHubIO Kubernetes documentation is available at [https://docs.bitnami.com/](https://docs.bitnami.com/). You can find there the following resources:

- [Documentation for PostgreSQL HA Helm chart](https://github.com/helmhub-io/charts/tree/main/helmhubio/postgresql-ha)
